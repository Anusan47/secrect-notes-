...
export default function ArchivePage() {
    ...
    const update = (updatedNote) =>
        setNotes((prev) => {
            if (!updatedNote.isArchived) {
                // remove unarchived notes instantly
                return prev.filter((n) => n._id !== updatedNote._id);
            }
    ...
    return (
        <div className="min-h-screen text-gray-100 flex flex-col bg-transparent">
            ...
            {/* Search + Sort Bar */}
            <motion.div
                className="max-w-4xl mx-auto w-full flex items-center justify-center gap-3 px-4 mb-10 flex-wrap"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
            ...
            <div className="flex-grow w-full max-w-6xl mx-auto px-4 mb-12">
                {loading ? (
                    ...
                    <motion.div
                        key="empty"
                        className="flex flex-col items-center justify-center text-gray-400 mt-24"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                    >
